...
/**
 * Created by fangke on 2015/6/24.
 */
public class SendCommentButton extends ViewAnimator implements View.OnClickListener {
    ...
    private Runnable revertStateRundle = new Runnable() {
        @Override
        public void run() {
    ...
    protected void onDetachedFromWindow() {
        removeCallbacks(revertStateRundle);
    ...
    public void setCurrentState(int state){
        ...
        if(state == STATE_DONE){
            setEnabled(false);
            postDelayed(revertStateRundle,RESET_STATE_DELAY_MILLIS);
